jQuery( document ).ready( function(){
    ...
    MetafansMedia.controlMediaImage = wp.media({
        title: wp.media.view.l10n.addMedia,
        multiple: false,
        library: {type: 'image' }
    });

    MetafansMedia.controlMediaImage.on('select', function () {
        var attachment = MetafansMedia.controlMediaImage.state().get('selection').first().toJSON();
        MetafansMedia.insertImage( attachment );
    });

    MetafansMedia.controlMediaVideo = wp.media({
        title: wp.media.view.l10n.addMedia,
        multiple: false,
        library: {type: 'video' }
    });

    MetafansMedia.controlMediaVideo.on('select', function () {
        var attachment = MetafansMedia.controlMediaVideo.state().get('selection').first().toJSON();
        MetafansMedia.insertVideo( attachment );
    });

    MetafansMedia.controlMediaFile = wp.media({
        title: wp.media.view.l10n.addMedia,
        multiple: false
    });

    MetafansMedia.controlMediaFile.on('select', function () {
        var attachment = MetafansMedia.controlMediaFile.state().get('selection').first().toJSON();
        MetafansMedia.insertFile( attachment );
    });
    ...
    $( '.tophive-mt-media' ).on( 'click',  '.tophive--add', function( e ) {
        ...
        console.log( p );
        MetafansMedia.setPreview( p );
        MetafansMedia.controlMediaImage.open();
    } );

    $( '.tophive-mt-media' ).on( 'click',  '.tophive--remove', function( e ) {
        e.preventDefault();
        var p = $( this ).closest('.tophive-mt-media');
        MetafansMedia.remove(p);
    } );
