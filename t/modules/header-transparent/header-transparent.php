...
class MetafansCoreCustomizer_Module_Header_Transparent extends MetafansCoreCustomizer_Module_Base {
	...
	function assets() {
		if ( ! is_admin() ) {
			...
			$this->add_js( MetafansCoreCustomizer()->get_url() . '/modules/header-sticky/js/' . $file, 'header-sticky', true );
		}
	}

	function logo_classes( $classes ) {
		$logo_id    = tophive_metafans()->get_setting( 'header_logo_tran' );
	...
	function transparent_logo() {
		$logo_id           = tophive_metafans()->get_setting( 'header_logo_tran' );
		$logo_image        = tophive_metafans()->get_media( $logo_id, 'full' );
		$logo_retina       = tophive_metafans()->get_setting( 'header_logo_tran_retina' );
	...
	function config( $configs ) {
		...
		$render_logo_cb_el = array(
			Metafans_Customize_Layout_Builder()->get_builder_item( 'header', 'logo' ),
			'render',
		);
	...
	function row_classes( $classes, $row_id, $builder ) {
		if ( $builder->get_id() == 'header' ) {
			if ( $this->is_transparent() ) {
				if ( tophive_metafans()->get_setting( "header_{$row_id}_transparent" ) ) {
	...
	function is_transparent( $force = false ) {
		if ( self::$is_transparent === null || $force ) {
			...
			if ( $is_tran ) {
				...
				$display = tophive_metafans()->get_setting_tab( 'header_transparent_display_pages', 'display' );
				...
				if ( $hide ) {
					...
					$cover_settings = Metafans_Page_Header::get_instance()->get_settings();
				...
				if ( tophive_metafans()->is_using_post() ) {
					$id = tophive_metafans()->get_current_post_id();
					if ( $id ) {
...
new MetafansCoreCustomizer_Module_Header_Transparent();