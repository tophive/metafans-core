jQuery(document).ready( function( $ ) {
    ...
    MetafansFontUpload.controlFont = {};

    _.each( Metafans_Fonts_Settings, function( info, ext ){
        MetafansFontUpload.controlFont[ info['mime'] ] = wp.media({
            library: {
                type: info['mime']
            },
            title: info['modal'],
        ...
        MetafansFontUpload.controlFont[ info['mime'] ].on('select', function () {
            var attachment = MetafansFontUpload.controlFont[ info['mime'] ].state().get('selection').first().toJSON();
            MetafansFontUpload.insertFile( attachment );
        });
    } );
    ...
    $( document.body ).on( 'click',  '.font-upload-btn', function( e ) {
        ...
        if ( mime  ) {
            MetafansFontUpload.setPreview(p);
            MetafansFontUpload.controlFont[mime].open();
        }
    } );
