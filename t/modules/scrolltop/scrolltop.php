...
class MetafansCoreCustomizer_Module_Scrolltop extends MetafansCoreCustomizer_Module_Base {
	...
	function assets() {
		...
		$this->add_css();
		$this->add_js();
		$this->add_local_js_args( 'scrolltop_duration', tophive_metafans()->get_setting( "{$this->section}_duration" ) );
		$this->add_local_js_args( 'scrolltop_offset', tophive_metafans()->get_setting( "{$this->section}_offset" ) );
	}
	...
	function render() {
		 $section = 'scrolltop';
		$show = tophive_metafans()->get_setting( "{$section}_show" );
		if ( ! is_customize_preview() ) {
			if ( ! $show ) {
				return;
			}
		}
		$icon = tophive_metafans()->get_setting( "{$section}_icon" );
		$position = tophive_metafans()->get_setting( "{$section}_position" );
		$icon = wp_parse_args(
			$icon,
			array(
				'type' => '',
...
new MetafansCoreCustomizer_Module_Scrolltop();