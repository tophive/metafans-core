<?php
class MetafansCoreCustomizer_Module_Hooks_Admin {
	...
	function __construct() {
		...
		add_filter( 'manage_' . MetafansCoreCustomizer_Module_Hooks::get_instance()->post_type . '_posts_columns', array( $this, 'listing_column_head' ), 10 );
		add_action( 'manage_' . MetafansCoreCustomizer_Module_Hooks::get_instance()->post_type . '_posts_custom_column', array( $this, 'listing_column_content' ), 10, 2 );
	...
	function listing_column_content( $column_name, $post_ID ) {
		$data = MetafansCoreCustomizer_Module_Hooks::get_instance()->get_settings( $post_ID );
		switch ( $column_name ) {
			case 'hook':
				echo esc_html( $data['hook'] );
				break;
			case 'role':
				$basic = MetafansCoreCustomizer_Module_Hooks::get_instance()->get_basic_roles();
				$show = false;
				if ( isset( $basic[ $data['user_role'] ] ) ) {
					echo esc_html( $basic[ $data['user_role'] ] );
					$show = true;
				} else {
					$roles = MetafansCoreCustomizer_Module_Hooks::get_instance()->get_roles();
	...
	function readable_condition( $condition ) {
		...
		if ( 'archive' == $condition['name'] ) {
			...
			switch ( $condition['sub_name'] ) {
				...
				default:
					$name = '';
					$archives = MetafansCoreCustomizer_Conditional::get_instance()->get_all_archive();
			...
			switch ( $condition['sub_name'] ) {
				...
				case 'not_found404':
					...
					break;
				default:
					$all_singular = MetafansCoreCustomizer_Conditional::get_instance()->get_all_single();
					$name = '';
	 ...
	 */
	function tabs() {
		if ( get_post_type() != MetafansCoreCustomizer_Module_Hooks::get_instance()->post_type ) {
			return;
		}
		$data = MetafansCoreCustomizer_Module_Hooks::get_instance()->get_settings();
		?>
	...
	function metabox() {
		...
		$post_type = MetafansCoreCustomizer_Module_Hooks::get_instance()->post_type;
		...
		<p>
			...
			<select name="tophive_hook[user_role]">
				...
				<optgroup label="<?php esc_attr_e( 'Advanced', 'tophive-pro' ); ?>">
					<?php foreach ( MetafansCoreCustomizer_Module_Hooks::get_instance()->get_roles() as  $k => $l ) { ?>
	...
	function metabox_hooks_list( $post ) {
		...
		<p class="cmt-align-label">
			...
			<select class="tophive_hook_name" name="tophive_hook[hook]">
				...
				<?php foreach ( MetafansCoreCustomizer_Module_Hooks::get_instance()->get_hook_locations() as $group ) { ?>
	...
	function admin_scripts( $hook ) {
		...
		MetafansCoreCustomizer_Module_Hooks::get_instance()->is_assets = true;
		...
		wp_enqueue_style( 'select2', esc_url( MetafansCoreCustomizer()->get_url() ) . '/assets/css/select2.min.css', array(), false );
		wp_enqueue_style( 'tophive-pro-admin', esc_url( MetafansCoreCustomizer()->get_url() ) . '/assets/css/admin/admin' . $suffix . '.css', array(), false );

		wp_enqueue_script( 'select2', esc_url( MetafansCoreCustomizer()->get_url() ) . '/assets/js/select2.min.js', array( 'jquery' ), false );
		wp_enqueue_script( 'tophive-pro-condition', esc_url( MetafansCoreCustomizer()->get_url() ) . '/assets/js/admin/condition' . $suffix . '.js', array( 'jquery', 'underscore' ) );

		wp_enqueue_script( 'tophive-pro-hooks', esc_url( MetafansCoreCustomizer_Module_Hooks::get_instance()->get_url() ) . '/assets/js/admin-script' . $suffix . '.js', array( 'jquery', 'underscore', 'tophive-pro-condition', 'wp-theme-plugin-editor' ) );

		wp_localize_script(
			'jquery',
			'MetafansCoreCustomizer_Hooks',
			array(
		...
		MetafansCoreCustomizer_Conditional::get_instance()->scripts( $data['conditionals'] );

		add_action( 'admin_footer', array( MetafansCoreCustomizer_Conditional::get_instance(), 'form_tpl' ) );
	}
}
