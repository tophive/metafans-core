...
wp.ajax.send = function(action, options) {
	if (_.isObject(action)) {
		action.data["msid"] = MetafansCoreCustomizer_MS.id;
		action.data["builder_id"] = top._current_builder_panel;
	} else {
		options.data["msid"] = MetafansCoreCustomizer_MS.id;
...
function tophive_customize_ms_change_url(preview_url, msid) {
	...
	MetafansCoreCustomizer_MS.id = parseInt(MetafansCoreCustomizer_MS.id);
	if (MetafansCoreCustomizer_MS.id != msid) {
...
(function($, api) {
	...
	api.bind("save-request-params", function(e) {
		e.msid = MetafansCoreCustomizer_MS.id;
		...
		if (id === "header") {
			$(".tophive--cb-actions", el.container).prepend(
				'<a data-id="multiple_headers" class="focus-section button button-secondary" href="#">' +
					MetafansCoreCustomizer_MS.l10n.multiple_text +
					"</a>"
			);
			if (MetafansCoreCustomizer_MS.id > 0) {
				var button = $(
					'<a class="display-condition" href="#">' +
						MetafansCoreCustomizer_MS.l10n.display +
						"</a>"
				);
	...
	api.bind("ready", function() {
		$(".multiple-section-list").each(function() {
			...
			var data = {
				...
				msid: MetafansCoreCustomizer_MS.id
			};
				...
				success: function(res) {
					...
					if (editing.length) {
						...
						if (current_Params.msid) {
							if (current_Params.msid !== MetafansCoreCustomizer_MS.id) {
								var urlParser = _.clone(window.location);
								var queryParams = _.extend(
	...
	$(document).ready(function($) {
		...
		$document.on("click", ".multiple-section-action .button", function(e) {
			...
			if (data.title) {
				...
				data["_nonce"] = MetafansCoreCustomizer_MS._nonce;
				data["current_msid"] = MetafansCoreCustomizer_MS.id;
