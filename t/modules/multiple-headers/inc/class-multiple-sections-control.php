<?php
class Metafans_Customizer_Control_Multiple_Sections extends Metafans_Customizer_Control_Base {
	...
	public function enqueue() {
		if ( is_null( self::$_is_loaded ) ) {
			...
			wp_enqueue_script( 'tophive-pro-condition', esc_url( MetafansCoreCustomizer()->get_url() ) . '/assets/js/admin/condition' . $suffix . '.js', array( 'jquery' ), false, true );

			wp_enqueue_script( 'tophive-pro-multiple-sections', esc_url( MetafansCoreCustomizer()->get_url('/modules/multiple-headers') ) . '/js/control' . $suffix . '.js', array( 'tophive-customizer-control' ), false, true );
			$id = isset( $_REQUEST['msid'] ) ? absint( $_REQUEST['msid'] ) : 0;
			wp_localize_script(
				'tophive-pro-multiple-sections',
				'MetafansCoreCustomizer_MS',
				array(
					'id' => $id,
					'l10n' => array(
