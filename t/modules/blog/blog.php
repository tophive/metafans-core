...
class MetafansCoreCustomizer_Module_Blog extends MetafansCoreCustomizer_Module_Base {
		...
		foreach ( $blog_sections as $section ) {
			...
			if ( $layout == 'blog_masonry' ) {
				if ( is_numeric( $columns ) && $columns > 1 ) {
					...
					foreach ( Metafans_Customizer_Auto_CSS::get_instance()->media_queries as $d => $s ) {
						if ( isset( $columns[ $d ] ) ) {
							$n = $columns[ $d ];
	...
	function assets() {
		...
		$this->add_js( MetafansCoreCustomizer()->get_url() . '/assets/js/shuffle' . $suffix . '.js', 'shuffle.js', true );
		$this->add_js();
	}
	...
	function change_render_class() {
		return 'MetafansCoreCustomizer_Blog_Posts_Layout';
	}
	...
	function blog_config( $args = array() ) {
		...
		$config = array(
			...
			array(
				...
				'choices' => array(
					'blog_classic' => array(
						'img' => MetafansCoreCustomizer()->get_url() . '/assets/images/blog_classic.svg',
					),
					'blog_column' => array(
						'img' => MetafansCoreCustomizer()->get_url() . '/assets/images/blog_column.svg',
					),
					'blog_masonry' => array(
						'img' => MetafansCoreCustomizer()->get_url() . '/assets/images/blog_masonry.svg',
					),
					'blog_lateral' => array(
						'img' => MetafansCoreCustomizer()->get_url() . '/assets/images/blog_lateral.svg',
					),
				),
	...
	function render( $args = array() ) {
		...
		if ( have_posts() ) :
			$l = new MetafansCoreCustomizer_Blog_Posts_Layout();
			$l->render( $args );
