<?php
MetafansCoreCustomizer()->register_sub_module( 'MetafansCoreCustomizer_Module_WooCommerce_Booster', 'MetafansCoreCustomizer_Module_WC_Single_Product_Layout', array() );
class MetafansCoreCustomizer_Module_WC_Single_Product_Layout {
     ...
     * The instance method
     *
	 * @return MetafansCoreCustomizer_Module_WC_Single_Product_Layout
	 */
	...
	function setup() {
		$this->configs = array(
			...
			'layout'            => tophive_metafans()->get_setting( 'wc_single_layout' ),
			// top-medium || top-full || left
			'slider_layout'     => tophive_metafans()->get_setting( 'wc_single_gallery_slider' ),
			...
			'thumbs_in_gallery' => tophive_metafans()->get_setting( 'thumbs_in_gallery' ),
			// 1 || 0 // apply for medium only
			'thumbs_nav_number' => tophive_metafans()->get_setting( 'wc_thumbs_number' ),
			// 1 || 0 // apply for medium only
			'tab_position'      => tophive_metafans()->get_setting( 'wc_single_product_tab_position' ),

            // 1 || 0 // apply for medium only
			'grid_columns'      => tophive_metafans()->get_setting( 'wc_single_left_grid_columns', 'all' ),
			// 1 || 0 // apply for medium only
			'left_col_size'      => tophive_metafans()->get_setting( 'wc_single_layout_left_col_size' ),

			'breadcrumb'      => tophive_metafans()->get_setting( 'wc_single_layout_breadcrumb' ), // show breadcrumb  1||0
		);

		$slider_layout = tophive_metafans()->get_setting( 'wc_single_gallery_slider' );

		if ( $slider_layout == 'thumbnails_vertical' ) {
			$this->configs['thumbs_position'] = tophive_metafans()->get_setting( 'thumbs_pos_vertical' );
		} else {
			$this->configs['thumbs_position'] = tophive_metafans()->get_setting( 'thumbs_pos_horizontal' );
		}
	...
	function assets() {
		...
		MetafansCoreCustomizer_Module_WooCommerce_Booster::get_instance()->add_css( 'single-product-layouts' . $suffix . '.css' );
		MetafansCoreCustomizer_Module_WooCommerce_Booster::get_instance()->add_js( 'sticky-kit' . $suffix . '.js' );
		MetafansCoreCustomizer_Module_WooCommerce_Booster::get_instance()->add_js( 'single-product-layout' . $suffix . '.js' );
	...
	function wc_before_main_layout() {
		...
		if ( is_product() ) {
            ...
            if ( has_action( 'single_product_layout_top' ) ) {
                $class = 'media-only';
                if ( ! MetafansCoreCustomizer()->is_enabled_module( 'MetafansCoreCustomizer_Module_WC_Gallery_Slider' ) ) {
