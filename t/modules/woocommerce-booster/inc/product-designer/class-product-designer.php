...
class MetafansCoreCustomizerduct_Designer {
	...
	public function is_qv_enable() {
		return MetafansCoreCustomizer()->is_enabled_module( 'MetafansCoreCustomizer_Module_WC_Quick_View' );
	}
	 ...
	 * @param array                         $items
	 * @param Metafans_WC_Catalog_Designer $pd
	 * @return stringœ
	 */
	public function body_render( $html, $items, $pd ) {
		...
		if ( count( $layout ) ) {
			...
			foreach ( $layout as $pos => $string ) {
				if ( $string ) {
					...
					$h_mod = tophive_metafans()->get_setting( 'wc_cd_ph_body_' . $pos );
		...
		if ( $bottom ) {
			...
			$h_mod = tophive_metafans()->get_setting( 'wc_cd_ph_body_bottom' );
			if ( $h_mod == 1 || $h_mod === true ) {
				$classes[] = 'col-horizontal';
			} else {
				$col_classes[] = 'col-vertical';
	...
	private function render_media() {
		...
		$items = tophive_metafans()->get_setting( 'wc_cd_media_positions' );
		...
		foreach ( $rows as $row_id => $row ) {
			if ( ! empty( $row ) ) {
				...
				foreach ( $row as $col_id => $array_html ) {
					...
					$h_mod = tophive_metafans()->get_setting( "wc_cd_ph_media_{$row_id}_$col_id" );
					if ( $h_mod == 1 || $h_mod === true ) {
	...
	public function product__add_to_cart() {
		...
		global $product;
		$args    = array();
		$display = tophive_metafans()->get_setting( 'wc_cd_add_to_cart_type' );
	...
	public function product__quickview() {
		...
		$display = tophive_metafans()->get_setting( 'wc_cd_qv_type' );
		$classes = array(
			'button',
			'cd-btn',
			'cd-quick-view',
	...
	public function product__compare() {
		...
		$display = tophive_metafans()->get_setting( 'wc_cd_compare_type' );
		$classes = array(
			'cd-btn',
			'tophive-wc-compare',
			'display-' . $display,
	 ...
	 * @param MetafansCoreCustomizerduct_Designer $pd          Product catalog designer.
	 */
...
new MetafansCoreCustomizerduct_Designer();
