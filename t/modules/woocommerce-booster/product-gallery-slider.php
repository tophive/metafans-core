...
class MetafansCoreCustomizer_Module_WC_Gallery_Slider extends MetafansCoreCustomizer_Module_Base {
	...
	function assets() {
		...
		$slick_url = MetafansCoreCustomizer_Module_WooCommerce_Booster::get_instance()->get_url();
		$suffix    = tophive_metafans()->get_asset_suffix();

		MetafansCoreCustomizer_Module_WooCommerce_Booster::get_instance()->add_js( $slick_url . '/assets/slick/slick' . $suffix . '.js', false, true );

		MetafansCoreCustomizer_Module_WooCommerce_Booster::get_instance()->add_css( 'product-gallery-slider' . $suffix . '.css' );
		MetafansCoreCustomizer_Module_WooCommerce_Booster::get_instance()->add_js( 'product-gallery-slider' . $suffix . '.js' );
		$vertical_mod = tophive_metafans()->get_setting( 'wc_single_gallery_slider' ) == 'thumbnails_vertical' ? true : false;
		MetafansCoreCustomizer_Module_WooCommerce_Booster::get_instance()->add_local_js_args(
			'wc_product_gallery',
			array(
				...
				'show_thumbs'     => tophive_metafans()->get_setting( 'wc_single_gallery_slider' ) == 'hide_thumbnails' ? false : true,
				'enable_zoom'     => apply_filters( 'wc_single_gallery_enable_zoom', tophive_metafans()->get_setting( 'wc_single_gallery_enable_zoom' ) == 1 ? true : false ),
				'enable_lightbox' => tophive_metafans()->get_setting( 'wc_single_gallery_enable_lightbox' ) == 1 ? true : false,
