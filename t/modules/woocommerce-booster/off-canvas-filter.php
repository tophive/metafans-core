...
class MetafansCoreCustomizer_Module_WC_Off_Canvas_Filter extends MetafansCoreCustomizer_Module_Base {
	 ...
	 */
	function assets() {
		MetafansCoreCustomizer_Module_WooCommerce_Booster::get_instance()->add_css( 'off-canvas-filter.css' );
		MetafansCoreCustomizer_Module_WooCommerce_Booster::get_instance()->add_js( 'off-canvas-filter.js' );
		MetafansCoreCustomizer_Module_WooCommerce_Booster::get_instance()->add_local_js_args( 'wc_filter', array( 'shop' => get_permalink( wc_get_page_id( 'shop' ) ) ) );
	}
	...
	function display_sidebar() {
		...
		if ( $this->is_show_sidebar() ) {
			if ( is_active_sidebar( 'off-canvas-sidebar' ) ) {
				$position = tophive_metafans()->get_setting( 'wc_products_filter_sidebar' );
				if ( $position != 'left' ) {
					$position = 'right';
				}
