jQuery(document).ready(function ($) {
    ...
    var MetafansCoreCustomizerduct_Gallery = function ($target, args) {
        var mobileModWidth = 568 ; //768;
        this.$target = $($target);

        args = $.extend({}, MetafansCoreCustomizer_JS.wc_product_gallery, args );
        var enableSlider = args.enable_slider;

        args.slider = $.extend({}, MetafansCoreCustomizer_JS.wc_product_gallery.slider, args.slider );
        args.thumbs = $.extend({}, MetafansCoreCustomizer_JS.wc_product_gallery.thumbs, args.thumbs);
    ...
    $.fn.tophive_wc_product_gallery = function (args) {
        new MetafansCoreCustomizerduct_Gallery(this, args );
        return this;
    };

    if (typeof MetafansCoreCustomizer_JS.wc_product_gallery !== "undefined") {
        ...
        $(document).on('tophive_quickview_product_init', function (e, $content) {
            ...
            setTimeout(function () {
                $('.woocommerce-product-gallery', $content).each(function () {
                    MetafansCoreCustomizer_JS.wc_product_gallery_quick_view.enable_slider = true;
                    $(this).tophive_wc_product_gallery(MetafansCoreCustomizer_JS.wc_product_gallery_quick_view );
                });
            }, 500);
        });
