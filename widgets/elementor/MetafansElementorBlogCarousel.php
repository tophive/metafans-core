...
class MetafansElementorBlogCarousel extends \Elementor\Widget_Base {

    public function get_name(){
        return 'thelemblog-carousel';
    }
    public function get_title(){
        return esc_html__( 'Blog Carousel', WP_TH_CORE_SLUG );
    }
    	...
    	*/
		$this->start_controls_section(
			'th_blog_carousel_blog_content_section',
				[
					'label' => esc_html__( 'Blog Carousel', WP_TH_CORE_SLUG ),
		...
		$this->start_controls_section(
			...
			$this->add_control(
				...
				[
					'label' => esc_html__( 'Posts Per Slide', WP_TH_CORE_SLUG ),
					'type' => \Elementor\Controls_Manager::SELECT,
					'default' => 2,
					'options' => [
										...
										$html .= '<div class="swiper-slide th-blog-block ' . $displayflex .'">';
											if( $json_settings['select_blog_layout'] === 'thumb-left' ){
												...
												$html .= '<div class="ec-d-flex ec-align-items-end ec-flex-column">';
				                            		$html .= MetafansElementorBase::renderBlogThumb($json_settings['show_blog_thumb']);
												$html .= '</div>';

												/*--------- Blog Content ---------*/
												$html .= '<div>';
													/*--------- Blog Single body  ---------*/
													$html .= MetafansElementorBase::getBlogBody( $json_settings, get_the_ID() );
													/* ---------Footer -----------*/
													$html .= MetafansElementorBase::getBlogFooter($json_settings['display_footer_author'], $json_settings['blog_read_more_text'], $json_settings['display_footer_view']);
												$html .= '</div>';
