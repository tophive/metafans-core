...
class MetafansRecentPostsWidget extends WP_Widget {
 
    public function __construct() {
        $widget_options = array(
            'classname' => 'tophive-mc-recent-post-widget',
            'description' => esc_html__( 'Fundocean Recent Posts', 'WP_TH_CORE_SLUG' )
        );
        ...
        return '<a class="theme-primary-color-head-hover text-decoration-none" href="'. get_permalink( $id ) .'">
            <div class="ec-d-flex ec-mb-3">
                ...
                <div class="ec-w-75 '. $spacing .'">
                    ...
                    <span class="mc-blog-readmore">'. esc_html__( 'Read more', WP_TH_CORE_SLUG ) .'<i class="ti-arrow-right"></i></span>
                </div>
            </div>
        </a>';
    }        
                ...
                if( $post_layout == 'one-big-col' ){
                    if($post_query->current_post == 0){
                        $html .= '<div class="display-date-'. $display_date .' display-more-'. $display_more .' ec-col-12 th-blog-big-head theme-primary-color-head-hover">';
                            ...
                            $html .= '<span class="mc-blog-readmore">'. esc_html__( 'Read more', WP_TH_CORE_SLUG ) .'<i class="ti-arrow-right"></i></span>';
                        $html .= '</div>';
                    }else{
    ...
    public function form( $instance ) {
        ...
        ?>
            <div class="tophive_fo_recent_post_widget_form">
                <p>
                    <label 
                        for="<?php echo $this->get_field_id( 'title' ); ?>"><?php echo esc_html__( 'Title : ', WP_TH_CORE_SLUG ); ?></label>
                    <input 
                ...
                <p>
                    <label 
                        for="<?php echo $this->get_field_id('post_count'); ?>"><?php echo esc_html__( 'Post Count', WP_TH_CORE_SLUG ); ?></label>
                    <input 
                        type="number"
                        min="3" 
                ...
                <p>
                    <label 
                        for="<?php echo $this->get_field_id('post_date'); ?>"><?php echo esc_html__( 'Show Date', WP_TH_CORE_SLUG ); ?></label>
                    <input 
                        type="checkbox"
                ...
                <p>
                    <label 
                        for="<?php echo $this->get_field_id('read_more'); ?>"><?php echo esc_html__( 'Show readmore', WP_TH_CORE_SLUG ); ?></label>
                    <input 
                        type="checkbox"
                ...
                <p>
                    <label 
                        for="<?php echo $this->get_field_id('post_layout'); ?>"><?php echo esc_html__( 'Post Count', WP_TH_CORE_SLUG ); ?></label>
                    <select 
                        name="<?php echo $this->get_field_name('post_layout') ?>" 
                        id="<?php echo $this->get_field_id('post_layout') ?>"
                    >
                        <option <?php selected( $post_layout , 'one-big-col'); ?>  value="one-big-col"><?php echo esc_html__( 'One Column + Big First', WP_TH_CORE_SLUG ) ?></option>
                        <option <?php selected( $post_layout , 'one-col'); ?>  value="one-col"><?php echo esc_html__( 'One Column', WP_TH_CORE_SLUG ) ?></option>
                        <option <?php selected( $post_layout , 'two-col'); ?>  value="two-col"><?php echo esc_html__( 'Two Column', WP_TH_CORE_SLUG ) ?></option>
                        <option <?php selected( $post_layout , 'three-col'); ?>  value="three-col"><?php echo esc_html__( 'Three Column', WP_TH_CORE_SLUG ) ?></option>
                    </select>
                </p>
                <p>
                    <label 
                        for="<?php echo $this->get_field_id('post_img_pos'); ?>"><?php echo esc_html__( 'Post Count', WP_TH_CORE_SLUG ); ?></label>
                    <select 
                        name="<?php echo $this->get_field_name('post_img_pos') ?>" 
                        id="<?php echo $this->get_field_id('post_img_pos') ?>"
                    >
                        <option <?php selected( $post_img_pos , 'img-top'); ?>  value="img-top"><?php echo esc_html__( 'Image top', WP_TH_CORE_SLUG ) ?></option>
                        <option <?php selected( $post_img_pos , 'img-left'); ?>  value="img-left"><?php echo esc_html__( 'Image left', WP_TH_CORE_SLUG ) ?></option>
                        <option <?php selected( $post_img_pos , 'img-right'); ?>  value="img-right"><?php echo esc_html__( 'Image Right', WP_TH_CORE_SLUG ) ?></option>
                    </select>
                </p>
            </div>
        <?php
